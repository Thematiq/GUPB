...
class Strategy(Protocol):
    def perform(
        self, champion: ChampionDescription, knowledge: ChampionKnowledge
    ) -> Action:
        pass
...
class FSMBot(Controller):
    ...
    def analyse_field(self, pos, visibility: Dict[Coords, TileDescription]):
        res = {"weapons": [], "enemies": [], "menhir": [], "effects": []}
    ...
    def should_attack(self, knowledge: ChampionKnowledge) -> bool:
        ...
        attacked_tiles = weapon_obj.cut_positions(
            tiles_map, knowledge.position, self.champ_data.facing
        )
        for tile in attacked_tiles:
            if (
                tile in knowledge.visible_tiles
                and knowledge.visible_tiles[tile].character is not None
            ):
                return True
        return False
